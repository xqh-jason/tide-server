...
4. **通用模块**：数据字典、操作日志、登录日志、文件上传（~~含断点续传~~已取消）、验证码、代码生成器、~~服务器监控~~（已取消）、定时任务。
...
| 文件上传 | ✅ W5 | 文件流、类型边界 |
| 断点续传 | ❌ 不做（2026-09-09 决定，代码已移除） | ~~分块、并发、异步 IO~~ |
| 图形验证码 | ✅ W5 | 图像生成、内存缓存 |
| 代码生成器 | ✅ W4 | 宏 / 泛型 / 模板 / 代码生成 |
| 服务器状态监控 | ❌ 不做（2026-09-09 决定） | ~~sysinfo、系统编程~~ |
| 定时任务 | ✅ W6 | tokio-cron-scheduler、异步调度 |
| 系统配置（参数配置）管理 | ✅ W5 | 通用 CRUD |
| 部门/职位管理 | 部门 ✅（2026-09-11）；职位进行中（2026-09-12 计划见 `plans/2026-09-12-position-management.md`） | 树结构 CRUD + 主数据 |
| 数据权限（5 档数据范围） | ◐ W7+ 可选（复杂度高，按需） | 数据范围过滤 |
| 表单生成器 | ❌ 不做（2026-09-09 决定） | ~~前端配置驱动~~ |
...
### W6 · 扩展模块（并发 / 系统 / 调度）
- ~~**断点续传**：分块上传、并发合并、异步 IO、分块哈希校验。~~（❌ 2026-09-09 决定不做）
- ~~**服务器状态监控**：sysinfo 采集 CPU/内存/磁盘/网络。~~（❌ 2026-09-09 决定不做）
- **定时任务**：tokio-cron-scheduler 周期任务 + 任务 CRUD（用生成器产出）。
- **交付**：三模块后端完成。

### W7 · 工程化部署
- ~~**表单生成器**（轻量，前端配置驱动）。~~（❌ 2026-09-09 决定不做）
...
| M5 | 六通用模块完成 | 第 5 周末 |
| M6 | 定时任务（断点续传、监控已取消） | 第 6 周末 |
| M7 | 部署上线（表单生成器已取消，全量完成） | 第 7 周末 |
