...
//! 作为**可复用的基座**对外暴露，供独立的业务仓库以自己的 binary 消费。
//!
//! # 为什么有 lib.rs（2026-09-18 新增）
//!
//! 本 crate 原先是 binary-only：`main.rs` 声明 6 个私有 `mod` 后直接起服务。
//! 那个形态对「单可部署二进制」是自洽的，但**无法被其它 crate 依赖** ——
//! 业务仓库（如 tide-hr）拿不到任何公开符号，只能 fork 整个仓库，从此两边分叉。
//!
//! 拆出 lib target 之后有两种消费方式：
//!
//! 1. **直接用基座起服务**：业务仓库的 `main.rs` 调 [`infra::app::run`]，
//!    自己的域通过 [`modules::register_domain`] 注册进来（见该函数文档）；
...
//! # 稳定性承诺
//!
//! 业务仓库以 **git 依赖 + 版本 tag** 消费本 crate。当前阶段只保证
//! 「同一 tag 内不变」，不承诺跨 tag 的 API 兼容性 —— 基座仍在演进，
//! 过早冻结公开面会阻碍它变好。业务仓升级 tag 时应预期到需要小改。
