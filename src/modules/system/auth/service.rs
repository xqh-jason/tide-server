...
use crate::modules::system::refresh_token::dto::CreateRefreshTokenParams;
...
pub async fn login(
    ...
    if let Err(err) = crate::modules::system::captcha::service::verify_captcha(
        cache,
        &req.captcha_id,
        &req.captcha_value,
    ) {
...
mod tests {
    ...
    fn login_req_with_captcha(cache: &MemoryCache, username: &str, password: &str) -> LoginReq {
        let resp = crate::modules::system::captcha::service::generate_captcha(cache).unwrap();
        let answer = cache
