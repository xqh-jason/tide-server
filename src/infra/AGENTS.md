...
**建档理由**：得分 10（`Config` 60 次 / 40 文件、`AppState` 166 次 / 30 文件的高中心化 + 独立领域：全仓唯一启动路径）。
...
| 全局共享状态 | `state.rs::AppState` | `config` / `db` / `cache` / `scheduler` 四件；`from_depot` 取值 |
| 路由挂载与中间件档位 | `router.rs::build` / `build_with` | 消费 `modules::DOMAINS` 或合并后的域表；`hoop_when` 挂超时豁免 |
...
- **`config.local.toml` 不存在**（2026-09-18 实测）：`.gitignore:10` 忽略了它、README 的 JWT 注释也提到「用 config.local.toml 覆盖」，但 `Config::load()` 只有 `add_source(File::with_name("config"))` 一个文件源，从不读它——写了它不会报错，也不会生效。本地覆盖统一用 `TIDE_*` 环境变量（如 `TIDE_SERVER__PORT=8087`）。
