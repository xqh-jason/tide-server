//! 假期额度批次过期作废任务（handler_name = `leave_grant_expire`）。
//!
...
//! 数据访问走 leave 域 service（跨域不直接操作 Entity）。
//!
...
pub const HANDLER_NAME: &str = "leave_grant_expire";
...
// → `crate::modules::biz::hr::leave::service::expire_grants_in_tx(&txn, today).await`
...
pub async fn run(state: &AppState) -> anyhow::Result<()> {
    let _ = state;
    Err(anyhow::anyhow!("未实现：leave_grant_expire::run"))
}
